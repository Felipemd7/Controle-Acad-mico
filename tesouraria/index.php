 ...
 <div id="relatorios">
   <ul>
    ...
    <li><strong>Mensalidades pagas em cartão de credito:</strong> R$ 
     <?php
     $sql_2 = mysqli_query($db,"SELECT SUM(valor) as soma FROM mensalidades WHERE dia_pagamento = '$d/$m/$a' AND metodo_pagamento = 'Cartão de crédito'");
	 	while($res_2 = mysqli_fetch_array($sql_2)){
			echo number_format($res_2['soma'],2);
		}
	 ?>    
    ...
    <li><strong>Mensalidades pagas em cartão de débito:</strong> R$ 
     <?php
     $sql_2 = mysqli_query($db,"SELECT SUM(valor) as soma FROM mensalidades WHERE dia_pagamento = '$d/$m/$a' AND metodo_pagamento = 'Cartão de débito'");
	 	while($res_2 = mysqli_fetch_array($sql_2)){
			echo number_format($res_2['soma'],2);
		}
	 ?>      
